...
import com.hq.modules.operate.service.MailService;
...
//        if(cfWarningrecord.getOperatorStatus() ==2 || cfWarningrecord.getOperatorStatus() ==1 ){
//            try {
//                mailService.sendSimpleMail("821509306@qq.com", "系统提示", "123123123");
//            } catch (Exception e) {
//                e.printStackTrace();
//            }
//        }
        if (cfWarningrecord != null) {
            if(cfWarningrecord.getOperatorStatus() == 2||cfWarningrecord.getOperatorStatus() == 1) {
                String sbName = cfWarningrecord.getEquipmentName();
                String equipmentId = cfWarningrecord.getEquipmentId();
                String warningReason = cfWarningrecord.getWarningReason();
                Integer a = cfWarningrecord.getOperatorStatus();

                // 对属性进行空值判断
                sbName = sbName != null ? sbName : "未知";
                equipmentId = equipmentId != null ? equipmentId : "未知";
                warningReason = warningReason != null ? warningReason : "未知";
                a = a != null ? a : -1;

                String reps = "您好，消防栓编号：" + equipmentId + " " + "消防栓名称：" + sbName + " " + "发生故障。 " + "故障描述：" + warningReason + "。  " + "请到系统平台进行处理";
//            System.out.println("dxx" + sbName + a + equipmentId);

                mailService.sendSimpleMail("自己qq@qq.com", "系统提示", reps);
            }
        } else {
            System.out.println("cfWarningrecord对象为空，无法生成故障通知。");
        }
