...
export default {
  ...
  created() {
    this.getPageList()
    this.getPersonList() // 获取巡查人列表
    this.fetchRobotList() // 获取机器人列表
  },
  methods: {
    // 获取巡查人列表
    getPersonList() {
      this.$http({
        ...
        if (data && data.code === 0) {
          const userList = data.page.list.map(user => ({
            label: user.username, // 使用用户名作为显示文本
            value: user.userId,   // 使用用户ID作为值
            type: 'person'       // 标记为人员类型
          }))
          // 合并到现有personList中
          this.personList = [...this.personList, ...userList]
        } else {
          this.$message.error('获取巡查人列表失败')
        }
      }).catch(error => {
        console.error('获取巡查人列表失败:', error)
        this.$message.error('获取巡查人列表失败')
      })
    },
    ...
    fetchRobotList() {
      this.$http({
        ...
        if (data.success) {
          const robotList = data.data.map(robot => ({
            label: robot.robotCode,
            value: robot.robotCode,
            type: 'robot' // 标记为机器人类型
          }));
          // 合并到现有personList中
          this.personList = [...robotList, ...this.personList]
        }
      }).catch(error => {
