    ...
    <el-form :inline="true" :rules="dataRules"  ref="dataForm" :model="dataForm" label-width="150px">
			<el-form-item label="所在省：" prop="province">
			  <el-select v-model="dataForm.province"
			             @change="getCityList(dataForm.province)"
			             placeholder="选择设备所在省" style="width:185px">
			    <el-option
			...
			<el-form-item label="所在市：" prop="citycode">
			  <el-select v-model="dataForm.citycode"
			             @change="getDistrictList(dataForm.citycode)"
			             placeholder="选择设备所在市" style="width:185px">
			    <el-option
  ...
  export default {
        ...
        dataRules:{
          ...
          equipmentNum :[
            {required: true, message: '设备数量不能为空', trigger: 'blur' }
					...
					province :[
					  {required: true, message: '设备所属省不能为空', trigger: 'blur' }
					],
					citycode :[
					  {required: true, message: '设备所属市县不能为空', trigger: 'blur' }
					],
          district :[
    ...
    methods: {
      ...
      menuListTreeCurrentChangeHandle (data, node) {
        ...
        this.$http({
          url:this.$http.adornUrl('/location/provinceList'),
          method: 'get'
        }).then( ({data}) =>{
          this.provinceList = data.page
        })
      },
			getCityList(province){
        this.dataListLoading = true;
        this.$http({
          url:this.$http.adornUrl('/location/cityList'),
          method: 'get',
					params:this.$http.adornParams({
					  province:province
					})
