...
public class LocationController {
	...
	public R save(@RequestBody LocationEntity cfLocation) {
		...
		//如果城市为空，则说明是直辖市
		cfLocation.setCity(StringUtils.isBlank(cfLocation.getCity()) ? cfLocation.getProvince() : cfLocation.getCity());
	...
	public R delete(@RequestBody String[] locationIds) {
		...
		for (String locationId : locationIds) {
			EquipmentEntity equipmentEntity = equipmentService.getByLoactionId(locationId);
			if (equipmentEntity != null) {
				return R.error("设备-" + equipmentEntity.getEquipmentName() + "-与要删除的位置信息存在关联关系，请先删除设备！");
			}
		}
	...
	public R provinceList() {
		List<LocationEntity> list = locationService.provinceList();
	...
	@ResponseBody
	@RequiresPermissions("operate:location:list")
	public R cityList(String province) {
		List<LocationEntity> list = locationService.selectCityList(province);
		return R.ok().put("page", list);
	}
